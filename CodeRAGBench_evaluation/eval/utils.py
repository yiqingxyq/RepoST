...
class TokenizedDataset(IterableDataset):
    ...
    def __iter__(self):
        ...
        for sample in range(self.limit_start, self.limit_start + self.n_tasks):
            ...
            # if self.tokenizer.name_or_path == "deepseek-ai/deepseek-coder-33b-instruct":
            #     prompt_contents = self.tokenizer.apply_chat_template(
        ...
        for s in generated_tokens:
            ...
            if postprocess:
                code_gens[sample].append(
                    task.postprocess_generation(gen_code, int(sample) + limit_start, new_tokens_only=False)
                )
            else:
                warnings.warn(
...
def extract_code_pieces(text: str, prefix: str = "```python", return_all: bool = False) -> str:
...
def extract_generation_code(output: str, question: str):
    try:
        ...
        print("Failed to extract code block with error `{}`".format(
            ex
        ))
